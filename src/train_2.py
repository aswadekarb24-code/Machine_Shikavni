...
def train(
    ...
    print(f"Training : {model_name} on {datanm} Dataset, noise = {useprob}")
    ...
    model = get_model(model_name, in_channels=in_channels, n_classes=nclasses, pretrained=pretrained).to(DEVICE)
    ...
    # Checkpoint and history path isolation by model name
    modelpath = os.path.join(CHECKPOINT, f'{model_name.lower()}_{datanm.lower()}_{suffix}.pth')
    historyfile = os.path.join(RESULTS, f"history_{model_name.lower()}_{datanm.lower()}_{suffix}.csv")
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"[{model_name.upper()}] Epoch: {epoch:03d}/{epochs:03d}")
        print(f"Train loss : {trloss:.4f} | Train acc : {tracc:.4f} | Train f1 : {trf1:.4f}")
        print(f"Test loss  : {tstloss:.4f} | Test acc  : {tstacc:.4f} | Test f1  : {tstf1:.4f}")

        if tstf1 > bestf1:
            bestf1 = tstf1
            torch.save(model.state_dict(), modelpath)
            print(f"New best checkpoint saved: {modelpath} | F1 = {bestf1:.4f}")
